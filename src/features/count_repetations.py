...
bench_set = bench_df[bench_df["set"] == bench_df["set"].unique()[8]]
squat_set = squat_df[squat_df["set"] == squat_df["set"].unique()[8]]
row_set = row_df[row_df["set"] == row_df["set"].unique()[3]]
ohp_set = ohp_df[ohp_df["set"] == ohp_df["set"].unique()[5]]
...
def count_reps(dataset, cutoff=0.4, order=10, column="acc_r"):
    ...
    plt.title(f"{excercise} - {category}: {len(peaks)} Reps")
    plt.show()
    """
...
count_reps(squat_set, cutoff=0.45)
count_reps(row_set, cutoff=0.6, column="gyr_x")
count_reps(ohp_set, cutoff=0.5)
...
accuracy = (rep_df['reps'] == rep_df['reps_pred']).sum() / len(rep_df) * 100